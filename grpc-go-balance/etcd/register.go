...
func Register(name string, host string, port int, target string, interval time.Duration, ttl int) error {
	...
	if err != nil {
		return fmt.Errorf("balance: create etcd2 client failed: %v", err)
	}
			...
			if err != nil {
				if etcd2.IsKeyNotFound(err) {
					if _, err := keysAPI.Set(context.Background(), serviceKey, serviceValue, &etcd2.SetOptions{TTL: time.Second * time.Duration(ttl)}); err != nil {
						log.Printf("balance: set service '%s' with ttl to etcd2 failed: %s", name, err.Error())
					}
				} else {
					log.Printf("balance: service '%s' connect to etcd2 failed: %s", name, err.Error())
				}
			} else {
				// refresh set to true for not notifying the watcher
				if _, err := keysAPI.Set(context.Background(), serviceKey, "", setOptions); err != nil {
					log.Printf("balance: refresh service '%s' with ttl to etcd2 failed: %s", name, err.Error())
				}
			}
			select {
			case <-stopSignal:
...
func UnRegister() error {
	...
	if err != nil {
		log.Printf("balance: deregister '%s' failed: %s", serviceKey, err.Error())
	} else {
		log.Printf("balance: deregister '%s' ok.", serviceKey)
	}
	return err
}
