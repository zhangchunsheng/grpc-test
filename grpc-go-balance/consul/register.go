...
func Register(name string, host string, port int, target string, interval time.Duration, ttl int) error {
	...
	if err != nil {
		return fmt.Errorf("balance: create consul client error: %v", err)
	}
	...
	go func() {
		...
		x := <-ch
		log.Println("balance: receive signal: ", x)

		err := client.Agent().ServiceDeregister(serviceID)
		if err != nil {
			log.Println("balance: deregister service error: ", err.Error())
		} else {
			log.Println("balance: deregistered service from consul server.")
		}

		err = client.Agent().CheckDeregister(serviceID)
		if err != nil {
			log.Println("balance: deregister check error: ", err.Error())
		}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				log.Println("balance: update ttl of service error: ", err.Error())
			}
		}
	}()
	...
	if err != nil {
		return fmt.Errorf("balance: initial register service '%s' host to consul error: %s", name, err.Error())
	}
	...
	if err != nil {
		return fmt.Errorf("balance: initial register service check to consul error: %s", err.Error())
	}
